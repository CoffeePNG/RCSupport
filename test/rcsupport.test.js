...
test('reporter, bot, and Discord-native messages do not notify the reporter', () => {
  assert.equal(shouldNotifyReporter(plugin, 'reporter', false), false);
  assert.equal(shouldNotifyReporter(plugin, 'staff', true), false);
  assert.equal(shouldNotifyReporter(native, 'staff', false), false);
  assert.equal(shouldNotifyReporter(plugin, 'staff', false), true);
});
