...
The `src/rcsupport` module is separate from the existing ticket pipeline. It uses one existing **staff-only Forum channel** with tags named exactly `open`, `acknowledged`, `in_progress`, `resolved`, and `wontfix`; the bot prepares missing tags at boot without removing custom tags. Select an empty Forum with the setup command and the bot configures all five tags. Existing channel permissions are preserved. The existing bug-report type in this repository is keyed `bug_report`, and its current `ticket_leads` assignments supply individual mentions when the Paper plugin's `alerts.mode` is `leads`. In `broadcast` mode, the Paper plugin alerts players with `rcsupport.alert` and neither kind of Forum post pings leads.
