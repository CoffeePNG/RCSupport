...
export async function confirmThreadDeletion(interaction: ChatInputCommandInteraction, forum: RCSupportForum): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  const postId = interaction.options.getString("thread") ?? interaction.channelId;
  try {
    if (!/^\d{17,20}$/.test(postId)) throw new Error("Use a report thread ID, or run this command inside that report thread.");
    const target = await forum.deletionTarget(interaction.guildId!, postId, interaction.user.id);
    ...
    await forum.deleteReportThread(interaction.guildId!, postId, interaction.user.id);
