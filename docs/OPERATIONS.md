...
- **One existing ticket pipeline.** Every configured ticket type (application, bug
  ...
  create → claim → close pipeline in `src/features/tickets/ticketHandler.ts`. What
...
(`src/features/tickets/ticketFormatter.ts`). `optionDescription` is the short blurb shown
...
### Full saved case history (bridge 1.0.8)

Minecraft `/bugreport history <id> [page]` reads the saved conversation privately with five entries per page and clickable navigation. This bot now captures both reporter and staff messages in mapped report threads. Attachment URLs and sticker names are included. Automated/webhook/system Discord messages are excluded; report status changes already have their own stored entries. Only new live staff replies trigger the existing reporter notification.
